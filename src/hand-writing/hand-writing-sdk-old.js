...
function drawEraser(context, stroke) {
  ...
  try {
    ...
    if (stroke.type === 'eraser') {
      var width = stroke.width * stroke.s;
      ...
      var pointerWidth = stroke.pointerWidth * stroke.s;
      var pointerHeight = stroke.pointerHeight * stroke.s;
...
function drawCurrentEraser(context, stroke) {
  ...
  try {
    ...
    if (stroke.type === 'eraser') {
      ...
      if (length && renderedLength === 0) {
        curvePath = new Path2D();
        drawRoundedRect(curvePath, firstPoint.x, firstPoint.y, stroke.pointerWidth, stroke.pointerHeight, stroke.radius);
        stroke.r.push(1);
      } else {
        ...
        for (var _i2 = renderedLength - 1; _i2 < length - 1; _i2++) {
          ...
          drawRoundedStroke(curvePath, currPoint, _nextPoint2, stroke.pointerWidth, stroke.pointerHeight, stroke.radius);
          stroke.r.push(1);
        }
      }
...
function resizeStroke(context, stroke) {
  ...
  stroke.x = stroke.x.map(function (x, i) {
    return x * pageWidth / width;
  });
  stroke.y = stroke.y.map(function (y, i) {
...
function drawStroke(context, stroke) {
  ...
  var width = stroke.penWidth * Math.pow(stroke.s, 0.5);
  var length = stroke.x.length;
  ...
  createClass(Editor, [{
    ...
    value: function setGroup() {
      ...
      if (oldGroup && oldGroup != newGroup) {
        var _Editor$groupMap$oldG = Editor.groupMap[oldGroup],
            editors = _Editor$groupMap$oldG.editors,
            undoRedoContext = _Editor$groupMap$oldG.undoRedoContext;
    ...
    value: function pointerDown(point) {
      ...
      if (this.checkStrokeType('eraser')) {
        ...
        // 初始化板擦
        var width = style.pointerWidth || style.penWidth || 20;
        var height = style.pointerHeight || Math.round(width * 1.25);
...
var HandWritingSDK = function () {
  ...
  function HandWritingSDK(_ref) {
    ...
    this.editor = MyWriting.register(dom, { render: 'canvas', group: group, observeResize: observeResize }, { ink: { color: '#000000', penWidth: 1.6 } }, { type: type });
